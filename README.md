...
| **Dépenses** | Suivi pro/perso par catégorie (créable à la volée), dépenses ponctuelles et récurrentes (fréquence unifiée dans un seul formulaire), estimations mensuelle/annuelle, **carte « à venir ce mois »** (récurrentes planifiées pas encore facturées), projection des échéances dans le calendrier, camembert par catégorie, **bouton « masquer les montants »** (flou, pour captures d'écran) |
...
| **Connaissances & compétences** | Base hard/soft en **arbre** (profondeur libre, catégories, quick-add « en rafale » contextuel) — niveau 0→5, statut (à acquérir/en cours/maîtrisé), années d'expérience, version cible ; liaison aux **projets** (utilisée / à acquérir) et **entretiens** (stack demandée), **questions techniques d'entretien** (difficulté, statut de révision, rattachées aux compétences), bouton **« Travailler cette compétence »** (crée une tâche datée → Tâches/Calendrier/Google), intégration **Graph** (nœuds compétences & questions) |
| **Santé** | Blessures/maladies, consultations, remboursements sécu/mutuelle (en attente/reçu), intégration calendrier |
| **Calendrier** | Vues mois / semaine / jour (grille 24h), sync Google Agenda bidirectionnelle avec vérification de connexion (scope, token, ping API) et indicateur d'état persistant, **push automatique des tâches datées et jalons vers l'agenda dédié**, fenêtre de synchro incrémentale (1 mois, extension à la navigation), drag-drop, événements journée entière & multi-jours en barres continues, raccourcis clavier (C/N, ↵), vue transversale (tâches, factures, jalons, interactions, renouvellements, entretiens, santé, dépenses récurrentes), **marqueur « G »** sur les événements importés de Google, **synchro auto throttlée** (seuil paramétrable dans les réglages) avec date de dernière synchro affichée |
| **Graph** | Visualisation relationnelle force-directed 2D — Source fiscale → Société → Contacts → Projets → Factures/Devis/Revenus, **reventes regroupées par plateforme** (Reventes → Vinted/Leboncoin/Momox → ventes, nœud ♻ teal distinct, filtrable). Expand/collapse, filtres par type, recherche, badges de statut, panneau détail, liens colorés par profondeur, particules animées |
| **Notifications** | Cloche temps réel, dropdown, marquage lu individuel/global, **notifications push Web Push** (PWA, y compris iOS installée) |
| **PWA / Mobile** | Application installable (manifest, icônes, plein écran iOS, bandeau d'installation à la connexion mobile), accueil mobile dédié (grille des modules, recherche, strip "Aujourd'hui", cartes "à traiter"), quick-add "+" central avec bottom sheet de saisies express, ergonomie tactile auditée |
...
| Investissements | `InvestmentPlatform`, `InvestmentEntry` (`capital` nullable = relevé de valorisation vs dépôt/retrait) |
| Fiscal / URSSAF | `UrssafDeclaration`, `UrssafDeclarationLine` |
| Entretien | `JobApplication`, `JobApplicationEvent` |
| Compétences | `Skill` (arbre self-relation `parentId`), `ProjectSkill`, `JobApplicationSkill`, `InterviewQuestion`, `QuestionSkill` |
